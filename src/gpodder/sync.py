...
try:
    ...
    log('(ipodsync) Could not find gpod')
...
class Device(services.ObservableService):
    ...
    def add_tracks(self, tracklist=[]):
        ...
        for id, track in enumerate(tracklist):
            ...
            if track.is_played() and gl.config.only_sync_not_played:
                continue
...
class iPodDevice(Device):
    ...
    def get_all_tracks(self):
        ...
        for track in gpod.sw_get_playlist_tracks(self.podcasts_playlist):
            t = SyncTrack(track.title, libgpodtrack=track)
            tracks.append(t)
        return tracks
    ...
    def add_track(self, episode):
        ...
        if libconverter.converters.has_converter(extension):
            ...
            callback_status = lambda percentage: self.notify('sub-progress', percentage)
...
class MP3PlayerDevice(Device):
    ...
    def copy_file_progress(self, from_file, to_file):
        ...
        while s:
            ...
            try:
                ...
                self.cancel()
                return False
            self.notify('sub-progress', float(bytes_read)/float(bytes))
            s = in_file.read(self.buffer_size)
        out_file.close()
        in_file.close()
    ...
    def get_all_tracks(self):
        tracks = []
        for filename in glob.glob(os.path.join(self.destination, '*', '*')):
            (title, extension) = os.path.splitext(os.path.basename(filename))
            t = SyncTrack(title, filename=filename)
            tracks.append(t)
        return tracks
