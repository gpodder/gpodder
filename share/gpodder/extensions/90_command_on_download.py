...
class gPodderExtension:
    ...
    def show_preferences(self):
        ...
        title = Gtk.Label(use_markup=True, label=_('<b><big>Command On Download Complete Extension</big></b>'))
        title.set_halign(Gtk.Align.CENTER)
        box.add(title)

        whatisthis = Gtk.Label(use_markup=True, wrap=True, label=_(
            'This extension defines a command to run upon episode download completion.'
        ))
    ...
    def on_preferences(self):
        return [(_('CmdOnDwnld'), self.show_preferences)]
